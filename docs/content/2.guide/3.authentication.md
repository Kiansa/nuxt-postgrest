...
If [`nuxt-auth-utils`](https://github.com/atinux/nuxt-auth-utils) is installed, it's detected automatically. After your login flow mints a PostgREST JWT, store it in the session under `postgrest_token` (configurable with `auth.tokenKey`):
...
Set `auth.provider: 'none'` (the default when nuxt-auth-utils isn't installed) and pass the token yourself:
